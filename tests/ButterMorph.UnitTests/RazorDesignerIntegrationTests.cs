...
public sealed class RazorDesignerIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task ValidationDesignerRendersSeparateValidationDocumentEditor()
    {
        ...
        Assert.Contains("Add rule", html, StringComparison.Ordinal);
        ...
        Assert.Contains("Advanced rule", html, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("Add assertion", html, StringComparison.Ordinal);
    ...
    public async Task DesignerStaticAssetsRespondSuccessfully()
    {
        ...
        Assert.Contains("return value.length === 0 || value.every(isEmptyMetadataValue)", schemaBuilderScript, StringComparison.Ordinal);
    ...
    public async Task PlaygroundHomeRendersHostSimulation()
    {
        ...
        string hostScript = await client.GetStringAsync("/_content/ButterMorph.Web.Razor/buttermorph/buttermorph-host.js");
        Assert.Contains("window.ButterMorphHost", hostScript, StringComparison.Ordinal);
        Assert.Contains("resolveCenteredPopup", hostScript, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("function resolveCenteredPopup", schemaScript, StringComparison.Ordinal);
        ...
        string mappingDesignerScript = await client.GetStringAsync("/_content/ButterMorph.Web.Razor/buttermorph/designer.js");
        Assert.Contains("window.parent.postMessage", mappingDesignerScript, StringComparison.Ordinal);
        string schemaHostFlowHtml = await client.GetStringAsync("/buttermorph/payload-schema/designer" + QueryMarker() + "context=payload-frame-flow&popup=true&embedded=true");
        Assert.Contains("window.parent.postMessage", schemaHostFlowHtml, StringComparison.Ordinal);
...
""";
    ...
    // Creates a schema with a numeric invoice total for validation designer tests.
    private static IStructureSchema CreateValidationDesignerSchema()
    {
        return new StructureSchema
        {
            Key = "invoice",
